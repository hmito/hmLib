...
namespace hmLib {
	namespace odeint{ 
		...
		public:
			state_validate_stepper(base_stepper Stepper_) : Result(validate_result::none), st(std::move(Stepper_)) {}
			void reset() {
				try_reset(st);
		...
		auto make_state_validate(stepper_ && Stepper_) {
			...
			return state_validate_stepper<stepper,stepper_category>(std::forward<stepper_>(Stepper_));
		}
	}
}
#
