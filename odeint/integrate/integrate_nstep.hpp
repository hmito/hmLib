#ifndef HMLIB_ODEINT_INTEGRATE_INTEGRATESTEP_INC
#define HMLIB_ODEINT_INTEGRATE_INTEGRATESTEP_INC 100
#
...
namespace hmLib {
	namespace odeint {
		namespace detail {
			...
			Time integrate_nstep(
				Stepper stepper, System sys, State& start_state,
				Time start_time, Time dt, unsigned int step_num,
		...
		template< class Stepper, class System, class State, class Time, class Observer >
		size_t integrate_nstep(Stepper stepper, System sys, State& start_state, Time start_time, Time dt, unsigned int step_num, Observer observer) {
			using stepper_category = typename boost::numeric::odeint::unwrap_reference<Stepper>::type::stepper_category;
			return detail::integrate_nstep(stepper, sys, start_state, start_time, dt, step_num, observer, stepper_category());
		}

		template< class Stepper, class System, class State, class Time>
		size_t integrate_adaptive(Stepper stepper, System sys, State& start_state, Time start_time, Time dt, unsigned int step_num) {
			using stepper_category = typename boost::numeric::odeint::unwrap_reference<Stepper>::type::stepper_category;
			return detail::integrate_nstep(stepper, sys, start_state, start_time,dt,step_num, boost::numeric::odeint::null_observer(), stepper_category());
		}
	}
}
#
