...
#	include <hmLib_v3_05/streambuf_interface.hpp>
#endif
#ifndef HMLIB_EXCEPTIONS_INC
#	include <hmLib_v3_05/exceptions.hpp>
#endif
namespace hmLib{
	class gate_not_opened_exception:public exceptions::io_not_opened{
		typedef exception excp_type;
		typedef exceptions::io_not_opened base_type;
	public:
		explicit gate_not_opened_exception(const std::string& Message_):excp_type(Message_.c_str()),base_type(Message_.c_str()){}
		explicit gate_not_opened_exception(const char* Message_):excp_type(Message_),base_type(Message_){}
	};
	class gate_opened_exception:public exceptions::io_opened{
		typedef exception excp_type;
		typedef exceptions::io_opened base_type;
	public:
		explicit gate_opened_exception(const std::string& Message_):excp_type(Message_.c_str()),base_type(Message_.c_str()){}
		explicit gate_opened_exception(const char* Message_):excp_type(Message_),base_type(Message_){}
	};
	...
	protected://common virtual functions
		pos_type iobuf_seekpos(pos_type abspos,std::ios_base::openmode which=std::ios_base::in|std::ios_base::out)override{
			hmLib_throw(exceptions::invalid_request,"basic_gatestreambuf can't do seek");
			return 0;
		}
	...
	protected://streambuf_interface ibuf
		...
		bool ibuf_unget(_Elem c)override{
			hmLib_assert(!bufflag,exceptions::invalid_request,"unget cannot use two times.");
			buf=c;
			bufflag=true;
			return false;
		}
