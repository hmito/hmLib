...
#	include<hmLib_v3_05/lab/mutex_proxy.hpp>
#endif
...
//			std::unique_ptr<mutex_proxy> pmx;
		...
		template<typename T>
		inquiries::connection connect(inquiry<T>& Inq_,inquiry<T>::func_type&& Func_){
			return Inq_.connect(std::move(Func_));
		}
		template<typename T,typename Mutex>
		inquiries::connection connect(inquiry<T>& Inq_,inquiry<T>::func_type&& Func_,Mutex& Mx_){
			return Inq_.connect(std::move(Func_),Mx_);
		}
		template<typename T>
		std::function<const T&(void)> make_sync(const T& value_){return [&](void)->const T&{return value_;};}
		template<typename T>
		inquiries::connection connect(inquiry<T>& Inq_,const T& Val_){
			return Inq_.connect(make_sync(Val_));
		}
		template<typename T,typename Mutex>
		inquiries::connection connect(inquiry<T>& Inq_,const T& Val_,Mutex& Mx_){
			return Inq_.connect(make_sync(Val_),Mx_);
		}
	}
}
#
