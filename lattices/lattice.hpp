...
namespace hmLib{
	...
	struct lattice {
	private:
		using this_type = lattice<T, dim_, is_const>;
	public:
	...
	public:
		...
		locator back_locate() { return locate(extent() + lattices::point<dim_>(-1)); }
		//!Return const locator of (size-1)
		const_locator back_locate()const { return locate(extent() + lattices::point<dim_>(-1)); }
	public:
	...
	public:
		...
		void resize(const extent_type& Extent_, const data_type& Data_) {
			...
			hmLib_assert(Data_.size() >= NewIndexer.lattice_size(), "The given Data is smaller than the lattice size.");
			Indexer = NewIndexer;
			Data = Data_;
		}
		...
		void resize(const extent_type& Extent_, input_iterator Begin, input_iterator End){
			indexer NewIndexer(Extent_);
			hmLib_assert(std::distance(Begin, End) >= NewIndexer.lattice_size(), "The given Data is smaller than the lattice size.");
			Indexer = NewIndexer;
