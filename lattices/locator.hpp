...
namespace hmLib{
	namespace lattices{
		...
		template<typename base_iterator_, typename indexer_>
		struct basic_sub_locator {
		private:
			using this_type = basic_sub_locator<base_iterator_, indexer_>;
			using base_iterator = base_iterator_;
			using indexer = indexer_;
		public:
			using value_type = typename std::iterator_traits<base_iterator>::value_type;
			...
			using extent_type = typename indexer::extent_type;
		public:
			basic_sub_locator() = default;
			basic_sub_locator(base_iterator Begin_, indexer Indexer_, point_type Pos_, indexer SubIndexer_, point_type SubPos_):Beg(Begin_), Indexer(Indexer_), Pos(Pos_), SubIndexer(SubIndexer_) , SubPos(SubPos_){}
			template<typename other_iterator,
				typename std::enable_if<
				std::is_convertible<other_iterator, base_iterator>::value
				&& !std::is_same<other_iterator, base_iterator>::value, std::nullptr_t
				>::type = nullptr
			>
			basic_sub_locator(const basic_sub_locator<other_iterator, indexer>& Other):Beg(Other.Beg), Indexer(Other.Indexer), Pos(Other.Pos), SubIndexer(Other.SubIndexer), SubPos(Other.SubPos) {}
			basic_sub_locator(const this_type&) = default;
			basic_sub_locator(this_type&&) = default;
			this_type& operator=(const this_type&) = default;
			this_type& operator=(this_type&&) = default;
			operator basic_locator<base_iterator, indexer>()const{
				return basic_locator<base_iterator, indexer>(Beg,Indexer,Pos);
			}
		public:
			point_type point()const { return Pos - SubPos; }
			index_type index()const { return SubIndexer(point()); }
			extent_type extent()const { return SubIndexer.extent(); }
		public:
			point_type base_point()const { return Pos; }
			index_type base_index()const { return Indexer(base_point()); }
			extent_type base_extent()const { return Indexer.extent(); }
