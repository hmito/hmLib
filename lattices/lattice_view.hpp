...
namespace hmLib{
	...
	private:
		using this_type = sublattice_view<iterator_, dim_>;
	public:
	...
	public:
		sublattice_view() = default;
		sublattice_view(const this_type&) = default;
		this_type& operator=(const this_type&) = default;
		sublattice_view(this_type&&) = default;
		...
		sublattice_view(const sublattice_view<other_iterator_, dim_>& Other):Begin(Other.Begin), OriginalIndexer(Other.OriginalIndexer), SubIndexer(Other.SubIndexer), Pos(Other.Pos) {}
		sublattice_view(iterator_base Begin_, const extent_type& OriginalExtent_, const point_type& OriginalPos_, const extent_type& SubExtent_)
			: Begin(Begin_)
	...
	public:
		...
		reference at(const point_type& Point_)const { return Begin[OriginalIndexer.index(Point_+Pos)]; }
	...
	public:
		...
		this_type& operator=(this_type&&) = default;
		template<typename other_iterator_, typename std::enable_if<std::is_convertible<other_iterator_, iterator_base>::value && !std::is_same<other_iterator_, iterator_base>::value, std::nullptr_t>::type CanConvertible = nullptr>
		lattice_view(const lattice_view<other_iterator_, dim_>& Other):Begin(Other.Begin), Indexer(Other.Indexer) {}
		lattice_view(iterator_base Begin_, const extent_type& Extent_):Begin(Begin_), Indexer(Extent_) {}
	public:
		//!Return reference of the elemtn at the given point with range check
		reference at(const point_type& Point_)const { return Begin[Indexer.index(Point_)]; }
		//!Return reference of the elemtn at the given elements point with range check
		template<typename... others>
		reference at(index_type Pos_, others... Others_)const {return at(lattices::point(Pos_, Others_...));}
		//!Return reference of the elemtn at the given point
	...
	auto make_lattice_view(iterator Begin, iterator End, const lattices::point_array_type<element_type, dim>& Extent) {
		...
		return lattice_view<iterator, dim>(Begin, static_cast<lattices::extent_type<dim>>(Extent));
	}
	...
	auto subview(const lattices::basic_locator<iterator, indexer>& Lat1, const lattices::basic_locator<iterator, indexer>& Lat2) {
		return sublattice_view<iterator, indexer::dim()>(
			Lat1.base_begin(),
			Lat1.base_extent(),
			Lat1.base_point(),
