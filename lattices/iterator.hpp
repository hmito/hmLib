...
namespace hmLib{
	namespace lattices{
		...
		public:
			...
			reference operator[](index_type Dif_)const { return Begin[Index+Dif_]; }
		public:
		...
		public:
			...
			this_type& operator=(this_type&&) = default;
			basic_sub_iterator(base_iterator Beg_, indexer OriginalIndexer_, point_type Pos_, extent_type Extent_, index_type SubIndex_)
				: Begin(Beg_)
		...
		public:
			point_type point()const{return SubIndexer.calc_point(SubIndex);}
			extent_type extent()const { return SubIndexer.extent(); }
			index_type index()const { return SubIndex; }
		public:
			...
			index_type base_index()const { return OriginalIndexer(base_point()); }
			base_iterator base_begin()const { return Begin; }
			base_iterator base()const { return Begin+base_index(); }
			reference operator*()const{ return base().operator*(); }
			pointer operator->()const { return base().operator->();}
			reference operator[](index_type Dif_)const { return Begin[OriginalIndexer(Pos+SubIndexer.calc_point(SubIndex+Dif_))]; }
		public:
