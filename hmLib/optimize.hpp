...
namespace hmLib{
	namespace optimize{
		...
		//hill climing search
		template<typename state, typename evaluate, typename mutate, typename urbg, typename breaker>
		auto hill_climbing_search(state& State, evaluate&& Evaluate, mutate&& Mutate, breaker&& Breaker, urbg&& URBG)->decltype(Evaluate(State)){
		...
		template<typename state, typename evaluate, typename mutate, typename temp, typename urbg, typename breaker>
		auto simulated_annealing_search(state& State, evaluate&& Evaluate, mutate&& Mutate, temp&& Temp, breaker&& Breaker, unsigned int StepNum, urbg&& URBG)->decltype(Evaluate(State)){
			auto Eval = Evaluate(State);
			state NState = State;
