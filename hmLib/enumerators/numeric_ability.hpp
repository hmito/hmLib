	...
	namespace enumerators{
		...
		struct accumulate_ability{
			...
			private:
				...
				using operation_type = std::function<result_type(result_type, const value_type&) >;
			public:
				result_type accumulate(result_type IniVal)override{ return std::accumulate(base::Cur, base::End, IniVal); }
				result_type accumulate(result_type IniVal, operation_type Operation)override{ return  std::accumulate(base::Cur, base::End, IniVal, std::move(Operation)); }
			};
			template<typename enumerator_traits>
			...
			public:
				virtual result_type accumulate(result_type IniVal) = 0;
				virtual result_type accumulate_sofar(result_type IniVal) = 0;
				virtual result_type accumulate_range(result_type IniVal) = 0;
				virtual result_type accumulate(result_type IniVal,operation_type Operation) = 0;
				virtual result_type accumulate_sofar(result_type IniVal, operation_type Operation) = 0;
				virtual result_type accumulate_range(result_type IniVal, operation_type Operation) = 0;
			};
			template<typename enumerator_traits, typename base>
			...
			public:
				result_type accumulate(result_type IniVal)override{ return std::accumulate(base::Cur, base::End, IniVal); }
				result_type accumulate_sofar(result_type IniVal)override{ return std::accumulate(base::Beg, base::Cur, IniVal); }
				result_type accumulate_range(result_type IniVal)override{ return std::accumulate(base::Beg, base::End, IniVal); }
				result_type accumulate(result_type IniVal, operation_type Operation)override{ return std::accumulate(base::Cur, base::End, IniVal, std::move(Operation)); }
				result_type accumulate_sofar(result_type IniVal, operation_type Operation)override{ return std::accumulate(base::Beg, base::Cur, IniVal, std::move(Operation)); }
				result_type accumulate_range(result_type IniVal, operation_type Operation)override{ return  std::accumulate(base::Beg, base::End, IniVal, std::move(Operation)); }
			};
		};
	}
}
